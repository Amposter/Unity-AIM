...
public class SimpleExperiment : INeatExperiment
{
    ...
    public IGenomeDecoder<NeatGenome, IBlackBox> CreateGenomeDecoder()
    {
		...
		{
			if (Config.substrateSetup == Config.HyperNEATSubstrateSetup.SingleObstacleInput) 
			{
				SubstrateNodeSet input = new SubstrateNodeSet (4);
				...
				//Sensors
				input.NodeList.Add (new SubstrateNode (inputID++, new double[] { 0, 2 })); // Angle WP
				input.NodeList.Add (new SubstrateNode (inputID++, new double[] { 2, 0 })); // Angle CO
				input.NodeList.Add (new SubstrateNode (inputID++, new double[] { 0, -2 })); // Dist CO
				input.NodeList.Add (new SubstrateNode (inputID++, new double[] { -2, 0 })); // Dist WP
